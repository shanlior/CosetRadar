      ...
      i = i_vec(j);
        if (size(Supp,1)~=0)
            for rr=1:size(Supp,1)
                for kk=1:Q
                %    if (Supp(rr,1)==i)  % mod(i+kk*g_N,N) || Supp(rr,1)==mod(i+kk*g_N,N)+1 || Supp(rr,1)==mod(i+kk*g_N,N)-1)
                    if (Supp(rr,1)== mod(i+kk*g_N,N)  || Supp(rr,1)==mod(i+kk*g_N,N)+1 || Supp(rr,1)==mod(i+kk*g_N,N)-1 ...
                           || Supp(rr,1)==mod(i+kk*g_N,N)+2 || Supp(rr,1)==mod(i+kk*g_N,N)-2 ...
                           || Supp(rr,1)==mod(i+kk*g_N,N)+3 || Supp(rr,1)==mod(i+kk*g_N,N)-3)
                        flag=1; 
                    end
                end
            end
